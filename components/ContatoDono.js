...
//
// Visual: rótulo só com o nome (o ícone do WhatsApp já diz do que se trata)
// e fundo verde translúcido com blur — mesma receita do botão de menu fixo
// do /admin (fundo /NN + ring-white/20 + backdrop-blur-sm), só que em verde.
// Os textos de instrução do Pix citam "o botão verde do WhatsApp com o
// nome …" (BlocoConfirmacaoPix, BlocoQrCodeAbacatePay): mudar a cor ou o
// rótulo daqui pede revisar aqueles textos também.
//
// Props:
...
export default function ContatoDono({ estabelecimento, nome = "a equipe", style }) {
  return (
    <a
      ...
      style={style}
      className="fixed bottom-4 right-4 z-40 flex items-center gap-2 rounded-full bg-green-600/70 px-4 py-3 font-medium text-white shadow-lg ring-1 ring-white/20 backdrop-blur-sm transition hover:bg-green-600/90"
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
