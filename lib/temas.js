...
const TEMAS_POR_SLUG = {
  laysla: {
    ...
    bgCampo: "#E9E7E3",
    bordaHeader: "#6B6B6B",
    // A borda escura acima quase some contra o botaoAdmin #545454: no admin
    // o toggle desligado (bg-border) ficava da mesma cor do ligado
    // (bg-primary). Cinza claro só no admin; ver app/[salon]/admin/page.js.
    bordaAdmin: "#C4C4C4",
    // Texto que senta direto no card escuro (--color-on-card, ver
    // app/[salon]/page.js). Sem ele, text-on-card cai no textoPrincipal.
    textoCard: "#F5F1EA",
    // O admin ignora textoCard (texto sempre textoPrincipal, escuro), então o
    // card #545454 dava texto escuro sobre cinza-escuro lá. Cinza claro só
    // no admin; o público segue com o card escuro. Mesmo campo da laryssa,
    // ver app/[salon]/admin/page.js.
    bgCardAdmin: "#F2F2F2",
    ...
    botaoHover: "#CFCFCC",
    textoBotao: "#191919",
    // O admin não define --color-on-primary e usa bg-primary+text-white fixo
    // (e text-primary como cor de texto), então o botão claro acima ficava
    // claro-sobre-claro lá. Cinza-escuro só no admin, mesmo recurso da layra;
    // ver app/[salon]/admin/page.js.
    botaoAdmin: "#545454",
    botaoAdminHover: "#454545",
    fonteDisplay: "font-cormorant",
    // Lockup completo de produção (monograma + nome + tagline numa imagem só,
    // 1300x330, ~3.94:1, fundo transparente) — substitui o par marca
    // (monograma) + marcaTexto (wordmark). O nome e a tagline já vêm na arte,
    // então ocultarNome evita duplicar.
    marca: "/images/laysla/laysla-logo-claro.png",
    layoutMarca: "centro",
    ocultarNome: true,
    // Altura de LAYOUT do lockup (64px / 80px no sm). O escalaLogo: 0.8
    // abaixo encolhe só o visual (~51px / 64px) — scale() não libera espaço,
    // então é esta altura, não a visível, que entra na conta do header.
    // object-contain: abaixo de ~347px de viewport o lockup (315px) é travado
    // pelo max-width e seria esticado na vertical; assim só encolhe.
    alturaMonograma: "h-16 w-auto object-contain sm:h-20",
    ...
    headerCompacto: true,
    // Header afinado (mesmo mecanismo da layra, ver Hero.js). Altura final =
    // logo de layout + py + borda (64+16+1 = 81px / 80+16+1 = 97px no sm);
    // min-h fica abaixo disso, só como piso. Medido no browser.
...
acolhe: {
  ...
  // Moldado no tenant-modelo `css` (staging id 8) e migrado pra cá; `css`
  // volta a não ter entrada (ver PROTOCOLO_DESENVOLVIMENTO.md).
  laryssa: {
    personalizado: true,
    bgHeader: "#000000",
    bgBody: "#FFFFFF",
    ...
    // Card preto (bgHeader) → texto branco direto sobre ele (--color-on-card).
    textoCard: "#FFFFFF",
    // O admin ignora textoCard (texto sempre textoPrincipal, preto), então o
    // card preto dava preto-sobre-preto lá. Cinza claro só no admin; o
    // público segue com o card preto. Ver app/[salon]/admin/page.js.
    bgCardAdmin: "#F2F2F2",
    botao: "#FF68AF",
    botaoHover: "#E85A9C",
    textoBotao: "#000000",
    marca: "/images/laryssa/laryssa-marca.png",
    marcaTexto: "/images/laryssa/laryssa-marcaTexto.png",
    layoutMarca: "esquerda",
    // Ponto de partida pra calibrar. headerMinH/headerPy só valem no ramo
    // headerCompacto (ver Hero.js). Conteúdo cabe no min-h: 84 - 2x12 = 60
    // (mobile), 96 - 2x12 = 72 (sm).
    headerCompacto: true,
    headerMinH: "min-h-[84px] sm:min-h-[96px]",
    headerPy: "py-3",
    alturaMonograma: "h-[60px] w-auto sm:h-[72px]",
    // Wordmark 413x51 (~8,1:1): a altura é limitada pela largura da coluna
    // ao lado do monograma (~206px mobile, ~325px sm), senão estica.
    alturaMarcaTexto: "h-[25px] sm:h-[38px]",
  },
