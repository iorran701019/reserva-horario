...
const TEMAS_POR_SLUG = {
  laysla: {
    ...
    personalizado: true,
    // Aprovado após comparação visual (Variação D): mesmo tom do bgBody,
    // ~9% mais claro (205 * 1.09 ≈ 223 → #DFDFDF) — sutil o bastante pra não
    // abrir contraste, mas já não é mais o creme (#FAF9F7) original.
    bgHeader: "#DFDFDF",
    ...
    // Aprovado após comparação visual (Variação D): as imagens da marca
    // (monograma + wordmark) vieram mais "alongadas" verticalmente do que o
    // desejado na fonte original — achatarLogo aplica scaleY(0.82) nelas via
    // CSS (sem reamostrar os PNGs). Campo genérico: qualquer tenant com o
    // mesmo problema de proporção pode reaproveitar (ver Hero.js).
    achatarLogo: true,
