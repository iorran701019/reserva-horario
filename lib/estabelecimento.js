...
export async function buscarEstabelecimento(slug) {
  const { data, error } = await supabase
    ...
    .select(
      "id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, aviso_regras_agendamento, cadastro_completo, granularidade_min, cancelamento_prazo_horas, link_localizacao"
    )
    .eq("slug", slug)
    .eq("ativo", true)
    .single();
