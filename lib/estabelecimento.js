...
export async function buscarEstabelecimento(slug) {
  const { data, error } = await supabase
    ...
    .select(
      "id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, aviso_regras_agendamento, cadastro_completo, exigir_endereco, granularidade_min, cancelamento_prazo_horas, link_localizacao, foto_perfil_url, foto_perfil_posicao, foto_perfil_zoom, rodape_selo1, rodape_selo2, rodape_selo3, fidelidade_ativa, fidelidade_meta_servicos, fidelidade_descricao_brinde, janela_agendamento_fim, msg_confirmacao, msg_lembrete, msg_cancelamento, msg_reativacao, msg_solicitacao_enviada, msg_duvida_generica, msg_cancelamento_cliente, msg_ajuda_prazo_expirado, msg_falha_cadastro, msg_contato_admin"
    )
    .eq("slug", slug)
    .eq("ativo", true)
    .single();
