...
// rodape_selo2, rodape_selo3, msg_* (10 colunas — ver MENSAGENS_WHATSAPP_CONFIG
...
// pedem o bloco de endereço (CEP/número/complemento/bairro/cidade/estado) —
// default true preserva o comportamento atual. `exigir_contato_emergencia`
// decide, independentemente, se pedem um "Contato de emergência (WhatsApp)"
// opcional — default false, mas backfilado por tenant como o inverso de
// exigir_endereco pra preservar o comportamento acoplado que existia antes
// (ver sql/estabelecimentos_exigir_contato_emergencia_instagram.sql).
// `exigir_instagram` decide se pedem o campo Instagram opcional — default
// true preserva o comportamento atual (campo incondicional).
...
export async function buscarEstabelecimento(slug) {
  const { data, error } = await supabase
    ...
    .select(
      "id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, aviso_regras_agendamento, cadastro_completo, exigir_endereco, exigir_contato_emergencia, exigir_instagram, granularidade_min, cancelamento_prazo_horas, link_localizacao, foto_perfil_url, foto_perfil_posicao, foto_perfil_zoom, rodape_selo1, rodape_selo2, rodape_selo3, fidelidade_ativa, fidelidade_meta_servicos, fidelidade_descricao_brinde, msg_confirmacao, msg_lembrete, msg_cancelamento, msg_reativacao, msg_solicitacao_enviada, msg_duvida_generica, msg_cancelamento_cliente, msg_ajuda_prazo_expirado, msg_falha_cadastro, msg_contato_admin"
    )
    .eq("slug", slug)
    .eq("ativo", true)
    .single();
