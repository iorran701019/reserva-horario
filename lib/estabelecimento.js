...
// granularidade_min } ou null (slug inexistente, inativo ou erro). Quem chama
...
// precisaSinal lá). `cadastro_completo` decide, por tenant, se
// IdentificacaoCliente exige o bloco de endereço completo ou só nome +
// WhatsApp (ver lá). `granularidade_min` é o passo (em minutos) da grade de
// horários no modo 'janela' — ver gerarSlotsDaJanela/calcularVagasPorHorario.
export async function buscarEstabelecimento(slug) {
  const { data, error } = await supabase
    .from("estabelecimentos")
    .select(
      "id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, cadastro_completo, granularidade_min"
    )
    .eq("slug", slug)
    .eq("ativo", true)
    .single();
