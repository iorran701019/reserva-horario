...
// link_localizacao, msg_* (10 colunas — ver MENSAGENS_WHATSAPP_CONFIG em
// lib/whatsapp.js) } ou null (slug inexistente, inativo ou erro). Quem chama
...
// precisaSinal lá). `aviso_regras_agendamento` é o texto do popup mostrado na
// etapa final de confirmação, no fluxo público (ver PopupRegrasAgendamento);
// null/vazio = nenhum popup.
...
export async function buscarEstabelecimento(slug) {
  const { data, error } = await supabase
    ...
    .select(
      "id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, aviso_regras_agendamento, cadastro_completo, granularidade_min, cancelamento_prazo_horas, link_localizacao, msg_confirmacao, msg_lembrete, msg_cancelamento, msg_reativacao, msg_solicitacao_enviada, msg_duvida_generica, msg_cancelamento_cliente, msg_ajuda_prazo_expirado, msg_falha_cadastro, msg_contato_admin"
    )
    .eq("slug", slug)
    .eq("ativo", true)
    .single();
