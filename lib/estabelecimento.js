...
// `lembrete_etiqueta_ativo` (boolean, default true) liga os dois gates de
// etiqueta ao confirmar/cancelar em Pendentes (ver comGateDeEtiqueta em
// page.js); só `false` desliga, ausente conta como ligado.
...
// `reserva_provisoria_expira_horas` é a ÚNICA exceção à regra dos dois selects
// abaixo: existe no banco mas de propósito NÃO entra em nenhum dos dois. Ela
// vale só pro rascunho abandonado no meio do wizard (finalizado = false) —
// bloco (a) de expirar_pendentes_vencidos, que lê a coluna direto no SQL, sem
// passar por estes loaders. Pendente que já apareceu na aba Pendentes expira
// pelo horário do serviço, não por essa coluna, e nada do lado do cliente
// consome o valor (o campo saiu de ConfiguracoesSalao, e a validade do QR Code
// Pix virou a constante EXPIRACAO_PIX_HORAS em
...
export async function buscarEstabelecimento(slug) {
  ...
  return await hidratarEstabelecimento(data);
}

// As DUAS hidratações, na ordem que importa: as regras especiais primeiro,
// porque o curto-circuito de hidratarAbacatepayConectado passou a consultá-las
// (um salão com o padrão desligado e uma regra de dezembro ainda precisa da
// credencial). Os dois loaders chamam esta função, nunca as duas soltas.
export async function hidratarEstabelecimento(estabelecimento) {
  return await hidratarAbacatepayConectado(
    await hidratarSinalRegrasEspeciais(estabelecimento)
  );
}

// Acrescenta `sinal_regras_especiais` (array, possivelmente vazio) ao objeto do
// estabelecimento — a entrada de resolverSinal (lib/sinalRegra.js), que decide
// se o atendimento cobra sinal e quanto.
//
// Consulta separada em vez de embed do PostgREST no select acima: aquele select
// é a lista canônica de colunas de `estabelecimentos` (regra do QA_CHECKLIST) e
// já é compartilhado entre dois arquivos; pendurar uma tabela filha nele
// misturaria duas coisas e obrigaria lib/perfil.js a aninhar um embed dentro de
// outro embed. Mesmo formato de hidratarAbacatepayConectado, pelo mesmo motivo.
//
// FALHA DE LEITURA VIRA LISTA VAZIA, e isso é o comportamento de hoje: sem
// regra especial nenhuma o resolver cai no padrão do salão (`sinal_regra` +
// `sinal_valor_centavos`), que é exatamente o que o app fazia antes desta
// feature existir. Diferente do fail-open da credencial, aqui "não sei" tem um
// default seguro e conhecido — e um erro que virasse exceção derrubaria o
// carregamento da página pública inteira por causa de uma exceção de preço.
export async function hidratarSinalRegrasEspeciais(estabelecimento) {
  ...
  if (error) {
    ...
    return { ...estabelecimento, sinal_regras_especiais: [] };
  }

  return { ...estabelecimento, sinal_regras_especiais: data ?? [] };
}
...
//
// O corte por `sinal_regra === 'desligado'` virou `salaoPodeCobrarSinal`
// (lib/sinalRegra.js) quando as regras especiais entraram: uma regra de
// período pode ligar a cobrança num salão cujo padrão está desligado (o caso
// "dezembro todo mundo paga"), e o corte antigo esconderia o QR Code
// justamente dele — a cascata rebaixaria pro manual e a dona veria a seção de
// Configurações em vermelho sem ter nada errado. Por isso esta função só pode
// rodar DEPOIS de hidratarSinalRegrasEspeciais (ver hidratarEstabelecimento).
//
...
export async function hidratarAbacatepayConectado(estabelecimento) {
  ...
  if (
    ...
    !salaoPodeCobrarSinal(estabelecimento)
  ) {
