...
// Nome da etapa anterior a exibir pra um item, com o padrão da UI quando o
// salão não deu um. O nome mora em dois lugares conforme a leitura: no admin
// vem do join `servicos(nome_etapa_anterior)` (item.servicos), e nas RPCs da
// cliente vem solto na raiz (comServicoAninhado só aninha `servico_nome`).
export function nomeEtapaAnterior(item) {
  return (
    item?.servicos?.nome_etapa_anterior?.trim() ||
    item?.nome_etapa_anterior?.trim() ||
...
// manda "HH:MM".
//
// `comPar` (linha com `reserva_grupo_id`): cancela pela RPC
// `agendamento_cancelar_cliente_par` (sql/rpc_cancelar_par.sql), que leva a
// irmã junto na mesma transação e devolve o mesmo status anterior. Sem par
// (o padrão) segue pela função de sempre, sem mudança.
export async function cancelarAgendamentoCliente({
  ...
  const { data: statusAnterior, error } = await supabase.rpc(
    comPar ? "agendamento_cancelar_cliente_par" : "agendamento_cancelar_cliente",
    { p_id: agendamentoId }
  );
