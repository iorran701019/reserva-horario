...
import { classificarAgendamento, ehAgendamentoConfirmadoFuturo } from "@/lib/particao";
...
export async function cancelarAgendamentoCliente({
  ...
  // Lê o status ANTES de cancelar, pra decidir se a dona precisa ser avisada.
  // Um agendamento ainda em "aguardando_sinal"/"pendente" nunca foi confirmado
  ...
  // (ou qualquer outro status) desfaz um compromisso já firmado, e esse
  // continua notificando como sempre.
  //
  // .single(): linha inexistente ou invisível por RLS vira erro, e aí não
  // cancelamos nada — mesmo tratamento (e mesma mensagem) do erro do update
  // abaixo, já que um update que não acha a linha também não cancelaria de
  // verdade.
  ...
  const cancelamentoSilencioso =
    atual.status === "aguardando_sinal" || atual.status === "pendente";

  // .select("id") pelo mesmo motivo do .single() na leitura acima: sem ele,
  // um update barrado por RLS volta com error null e ZERO linhas — a função
  // devolveria ok:true, o WhatsApp de cancelamento abriria e a cliente iria
  // embora achando que cancelou, com o horário ainda de pé na agenda da dona.
...
// "YYYY-MM-DD" + N dias -> "YYYY-MM-DD". Monta a data componente-a-componente
// (nunca new Date("YYYY-MM-DD"), que seria interpretada como UTC e deslocaria
// o dia em GMT-3) — mesma convenção de lib/horarios.js, lib/particao.js e
// somarDias em lib/manutencaoSugerida.js.
...
// confirmado + aguardando_sinal): "cancelado" nunca conflita, e deixar
// aguardando_sinal de fora abriria o furo óbvio de reservar, não pagar e
// reservar de novo no dia seguinte. Não existe status de falta/no-show neste
// projeto — os quatro acima são todos os valores gravados.
//
...
export async function buscarConflitoPrazoMinimo(
  ...
  // Janela consultada com o prazo cheio pros dois lados; o corte fino
  // (distância ESTRITAMENTE menor que o prazo) fica no JS logo abaixo, pra
  // que "7 dias de prazo" aceite dois agendamentos a exatos 7 dias um do
  // outro em vez de recusar por um dia de folga.
  const inicio = deslocarISO(dataAlvo, -dias);
  const fim = deslocarISO(dataAlvo, dias);
  ...
  const { data, error } = await supabase
    ...
    .eq("telefone", telefoneDigitos)
    .in("status", ["pendente", "confirmado", "aguardando_sinal"])
    .or("finalizado.eq.true,origem.eq.importado")
    .gte("data", inicio)
    .lte("data", fim)
    .order("data")
    .order("horario");
