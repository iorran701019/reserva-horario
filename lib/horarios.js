...
// Parte de HORA_ABERTURA e soma a duração do serviço enquanto
// (slot + duracaoMin) <= HORA_FECHAMENTO. Domingo (fechado) => [].
// duracaoMin vem do serviço escolhido; cai em DURACAO_MINUTOS se ausente.
export function gerarSlots(data, duracaoMin = DURACAO_MINUTOS) {
  if (!estaAberto(data)) return [];

  // Duração inválida (0, negativa, NaN) geraria laço infinito ou slots errados.
  const passo = Number(duracaoMin) > 0 ? Number(duracaoMin) : DURACAO_MINUTOS;
