...
// Gera os slots "HH:MM" dentro de UMA janela de trabalho.
// janela = { inicio: "13:00", fim: "18:00", almocoInicio: "15:00", almocoFim: "16:00" }
// almoco* são opcionais. Retorna [] se não houver janela (profissional não
// trabalha nesse dia). O passo é granularidadeMin (cai em GRANULARIDADE_MIN se
// ausente/inválido) — normalmente vem de estabelecimentos.granularidade_min.
export function gerarSlotsDaJanela(
  janela,
  duracaoMin = DURACAO_MINUTOS,
  granularidadeMin = GRANULARIDADE_MIN
) {
  if (!janela?.inicio || !janela?.fim) return [];

  const duracao = Number(duracaoMin) > 0 ? Number(duracaoMin) : DURACAO_MINUTOS;
  const granularidade =
    Number(granularidadeMin) > 0 ? Number(granularidadeMin) : GRANULARIDADE_MIN;
  const inicio = horaParaMinutos(janela.inicio);
  const fim = horaParaMinutos(janela.fim);

  // Almoço vira um intervalo bloqueado (se existir).
  const almocoIni = janela.almocoInicio ? horaParaMinutos(janela.almocoInicio) : null;
  const almocoFim = janela.almocoFim ? horaParaMinutos(janela.almocoFim) : null;

  const slots = [];
  for (let m = inicio; m + duracao <= fim; m += granularidade) {
    // Descarta o slot se ele invadir o almoço.
    const invadeAlmoco =
      almocoIni != null && m < almocoFim && almocoIni < m + duracao;
    if (!invadeAlmoco) slots.push(minutosParaHora(m));
  }
  return slots;
}
