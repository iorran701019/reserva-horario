...
import { classificarAgendamento, STATUS_SUCESSO } from "@/lib/particao";
...
export async function buscarClientes(estabelecimentoId) {
  const { data, error } = await supabase
    ...
    .select(
      ...
      "id, nome, whatsapp, nascimento, instagram, cidade, bairro, endereco, estado, contato_emergencia, etiqueta_id, etiquetas_cliente(nome, emoji, cor)"
    )
...
// do que já está fechado com a cliente. "concluido" também fica de fora
// sozinho (classificarAgendamento o manda pro histórico) — é o que garante
// que o CarrosselAgendamentos da ficha nunca mostre um atendimento já
// concluído. [] se não sobrar nenhum ou a consulta falhar (o helper já
// devolve [] em erro).
...
// Último atendimento CONCLUÍDO do cliente. Busca os 5 confirmados/concluídos
// mais recentes e filtra em JS com classificarAgendamento (só "historico" já
...
// Histórico completo do cliente (concluídos e cancelados), mais recente
// primeiro. Só o que já foi RESOLVIDO no banco: "confirmado" fica de fora de
// propósito — sem corte por horário, um confirmado FUTURO entraria aqui
// rotulado "Concluído", e um confirmado já passado mas ainda não concluído
// pertence a "Aguardando Conclusão" (aba Pendentes), não ao Histórico.
// valor_cobrado_centavos/nao_compareceu + o preço do serviço + o sinal
// (sinal_declarado_pago/sinal_valor_centavos) alimentam o botão "Editar" dos
// concluídos (ver CardConclusaoAtendimento). Erro de consulta devolve [].
export async function buscarHistoricoCompleto(estabelecimentoId, telefoneDigitos) {
  const { data, error } = await supabase
    .from("agendamentos")
    .select(
      "id, data, horario, status, servico_id, reserva_grupo_id, papel_reserva, expirado_automaticamente, valor_cobrado_centavos, nao_compareceu, sinal_declarado_pago, sinal_valor_centavos, forma_pagamento_servico, editado_manualmente_em, servicos(nome, preco_centavos, nome_etapa_anterior)"
    )
    .eq("estabelecimento_id", estabelecimentoId)
    .eq("telefone", telefoneDigitos)
...
// { nome, cor } ou null. A entrada existe pra todo cliente ENCONTRADO,
...
export async function buscarEtiquetasPorTelefones(estabelecimentoId, telefones) {
  ...
  const { data, error } = await supabase
    .from("clientes")
    .select("id, whatsapp, etiqueta_id, etiquetas_cliente(nome, emoji, cor)")
    .eq("estabelecimento_id", estabelecimentoId)
    .in("whatsapp", alvos);
...
// Quantos agendamentos CONFIRMADOS (ou já concluídos — ver STATUS_SUCESSO)
// o cliente já tem no salão. Alimenta o gate
...
export async function contarAgendamentosConfirmados(estabelecimentoId, telefone) {
  ...
  const { count, error } = await supabase
    ...
    .in("status", STATUS_SUCESSO)
    .eq("finalizado", true)
    // Etapa anterior de um par não é atendimento anterior da cliente: sem este
    // filtro, a "Cliente Nova" que acabou de marcar o par já dispararia o aviso
    // de "já tem atendimento". `.neq` sozinho descartaria as linhas de
    // papel_reserva NULL (a maioria), por isso o `.or`.
    .or("papel_reserva.is.null,papel_reserva.neq.anterior");
