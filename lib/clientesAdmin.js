...
export async function buscarClientes(estabelecimentoId) {
  const { data, error } = await supabase
    .from("clientes")
    .select(
      "id, nome, whatsapp, nascimento, instagram, cidade, bairro, endereco, estado, contato_emergencia"
    )
...
// (já filtra pendente/confirmado/aguardando_sinal, traz duracao_min e ordena
// por data/horário) mas o status cru não muda sozinho quando o horário passa
// — por isso filtra com classificarAgendamento antes de pegar o primeiro,
// senão um agendamento vencido (ainda confirmado no banco) apareceria como
// "próximo" no lugar do que realmente vem a seguir. null se não sobrar nenhum
// ou a consulta falhar (o helper já devolve [] em erro).
async function buscarProximoAgendamento(estabelecimentoId, telefoneDigitos) {
  ...
  const futuros = lista.filter((item) => classificarAgendamento(item) !== "historico");
  const proximo = futuros[0] ?? null;
  if (!proximo) return null;

  // Respostas do popup de perguntas do serviço (ver lib/agendamentoRespostas),
  // pro card "Próximo agendamento" da ficha do cliente.
  const respostas = await buscarRespostasPorAgendamento([proximo.id]);
  return { ...proximo, respostas: respostas.get(proximo.id) ?? [] };
}
