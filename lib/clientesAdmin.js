...
export async function buscarHistoricoCompleto(estabelecimentoId, telefoneDigitos) {
  const { data, error } = await supabase
    .from("agendamentos")
    .select(
      "id, data, horario, status, servico_id, expirado_automaticamente, valor_cobrado_centavos, nao_compareceu, sinal_declarado_pago, sinal_valor_centavos, forma_pagamento_servico, servicos(nome, preco_centavos)"
    )
    .eq("estabelecimento_id", estabelecimentoId)
    .eq("telefone", telefoneDigitos)
