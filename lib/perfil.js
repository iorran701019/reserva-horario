...
export async function buscarPerfil() {
  ...
  const { data, error } = await supabase
    .from("perfis")
    .select(
      "papel, estabelecimentos(id, nome, whatsapp, slug, fidelidade_ativa, fidelidade_meta_servicos, fidelidade_descricao_brinde, janela_agendamento_fim, msg_confirmacao, msg_lembrete, msg_cancelamento, msg_reativacao, msg_solicitacao_enviada, msg_duvida_generica, msg_cancelamento_cliente, msg_ajuda_prazo_expirado, msg_falha_cadastro, msg_contato_admin, msg_fora_da_janela)"
    )
    .eq("user_id", user.id)
    .single();
