import { supabase } from "@/lib/supabaseClient";
import { hidratarEstabelecimento } from "@/lib/estabelecimento";

// Resolução do PERFIL do usuário autenticado (tabela `perfis`), que decide QUAL
// salão o admin enxerga conforme o papel:
//   - 'global' → pode navegar entre salões pelo slug do PATH (comportamento
//     atual do /[salon]/admin); o estabelecimento do perfil é só o salão-casa,
//     usado como destino padrão do redirect pós-login.
//   - 'dono'   → preso ao próprio salão (perfis.estabelecimento_id), ignorando
//     qualquer slug da URL.
//
// A linha é filtrada por user_id = auth.uid() e traz o estabelecimento vinculado
// por join (id, nome, whatsapp, slug). O slug alimenta o redirect pós-login e o
// link de reagendamento do WhatsApp (que deve apontar pro salão REAL do perfil,
// não pro slug digitado na URL).

// Busca o perfil do usuário autenticado. Devolve:
//   - undefined → sem sessão (nenhum usuário autenticado)
//   - null      → autenticado, porém SEM linha em perfis (conta órfã)
//   - { papel, estabelecimento } → perfil resolvido; `estabelecimento` pode ser
//     null se o vínculo apontar pra salão inexistente.
export async function buscarPerfil() {
  const {
    data: { user },
  } = await supabase.auth.getUser();

  if (!user) return undefined;

  const { data, error } = await supabase
    .from("perfis")
    .select(
      "papel, estabelecimentos(id, nome, whatsapp, slug, sinal_regra, sinal_valor_centavos, sinal_chave_pix, metodo_cobranca_pix, granularidade_min, fidelidade_ativa, fidelidade_meta_servicos, fidelidade_descricao_brinde, janela_agendamento_fim, meses_alcance_edicao_agenda, prazo_minimo_entre_agendamentos_dias, antecedencia_minima_horas, cutoff_dia_seguinte_ativo, cutoff_dia_seguinte_hora, msg_confirmacao, msg_lembrete, msg_cancelamento, msg_reativacao, msg_solicitacao_enviada, msg_duvida_generica, msg_cancelamento_cliente, msg_ajuda_prazo_expirado, msg_falha_cadastro, msg_contato_admin, msg_fora_da_janela, msg_alteracao_data, servico_manutencao_externa_id, etiqueta_bloqueio_sinal_id, ocultar_preco_servicos, ocultar_duracao_servicos, pular_perguntas_adicionais_admin, lembrete_etiqueta_ativo, conclusao_manual_ativa, confirmado_expira_horas, cnpj)"
    )
    .eq("user_id", user.id)
    .single();

  if (error || !data) return null;

  // Regra fixa do projeto (QA_CHECKLIST.md): toda coluna nova de
  // `estabelecimentos` entra nos DOIS selects, este e o de
  // lib/estabelecimento.js. `sinal_chave_pix` e `sinal_valor_centavos` faltavam
  // aqui — o /admin lia só `sinal_regra` e `metodo_cobranca_pix` —, e sem a
  // chave não há como calcular o degrau manual da cascata pro card de
  // Pendentes. A hidratação (`abacatepay_conectado` e, desde as regras
  // especiais de sinal, `sinal_regras_especiais`) é a MESMA função do outro
  // loader, nunca uma segunda implementação — e a ordem das duas importa, por
  // isso `hidratarEstabelecimento` e não as duas soltas.
  return {
    papel: data.papel,
    estabelecimento: await hidratarEstabelecimento(data.estabelecimentos ?? null),
  };
}
