...
export async function calcularVagasPorHorario({
  ...
  // abaixo — usada pelo fluxo de perguntas de serviço (ver
  // calcularAjusteDuracao em FormularioAgendamento) pra refletir a duração
  // EFETIVA (base + ajustes das opções respondidas) no cálculo de slots e na
  // checagem de sobreposição. Sem efeito nos demais chamadores, que não
  // passam este parâmetro.
  ...
  // por quem chama (ver o parâmetro acima).
  let duracao;
  ...
  for (const profissionalId of profissionalIds) {
    ...
    aplicarExcecoes(conjunto, excecoes, duracao);
