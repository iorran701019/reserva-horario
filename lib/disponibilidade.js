...
// a data está além de estabelecimentos.janela_agendamento_fim (ver
...
export async function calcularVagasPorHorario({
  ...
  if (!dentroDaJanelaAgendamento(data, estab)) return mapa;
...
export function filtrarPorAntecedenciaMinima(horariosDisponiveis, data, estabelecimento) {
  ...
  for (const horario of Object.keys(horariosDisponiveis)) {
    if (ehHoje && construirDataHora(data, horario).getTime() < agora.getTime()) continue;

    if (antecedenciaAtiva) {
      const horasDeAntecedencia =
        (construirDataHora(data, horario).getTime() - agora.getTime()) / (1000 * 60 * 60);
      if (horasDeAntecedencia < Number(antecedenciaHoras)) continue;
    }

    if (cutoffBloqueiaODiaSeguinte && horario < LIMITE_MANHA_CORTE) continue;

    resultado[horario] = horariosDisponiveis[horario];
  }
  return resultado;
}
