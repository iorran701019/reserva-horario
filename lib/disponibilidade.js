...
async function carregarBaseDisponibilidade({
  ...
  if (profissionalIds.length === 0) return { ...vazio, estab, granularidade, duracao };
...
export async function calcularVagasPorHorario({
  ...
  const base = await carregarBaseDisponibilidade({
    ...
    // bloqueia o cálculo — o dia já foi liberado antes, no calendário (ver
    // CalendarioDias/modoLivre). No público, comportamento de sempre.
    dataParaJanela: contexto === "admin" ? null : data,
  });
  if (base.foraDaJanela || base.profissionalIds.length === 0) return mapa;
...
export async function calcularVagasDoMes({
  ...
  for (let d = 1; d <= diasNoMes; d++) {
    ...
    // Sem ninguém que atenda o serviço, ou dia além da janela de agendamento:
    // entra no resultado como dia sem vaga nenhuma (e não como dia ausente,
    // que significaria "não calculado").
    if (
