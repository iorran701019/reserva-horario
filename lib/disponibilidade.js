...
import {
  ...
  mesAgendavel,
  chaveMesJanela,
  marcarMesesJanelaIndisponiveis,
...
// Por último, tipo_registro='exclusividade_servico' RESTRINGE o próprio
// serviço consultado: com uma regra ativa na data, ele só fica nos horários
// marcados pra aquele dia da semana (ver aplicarExclusividades). Os outros
// serviços nunca são afetados, e nada é criado nem liberado.
//
...
// --- Fuso fixo do agendamento --------------------------------------------
//
// Toda decisão de "que horas são" nas regras de antecedência acontece em
// America/Sao_Paulo, NUNCA no fuso do processo. No navegador da cliente os
// dois coincidem; no servidor não — a Vercel roda em UTC, e ali `new Date()`
// com getters locais devolvia o dia/hora de Londres. Era isso que fazia a
// revalidação server-side (ver app/api/agendamentos/validar-antecedencia)
// recusar, no último clique, um horário que o wizard acabara de oferecer: a
// antecedência efetiva ficava 3h maior, e depois das 21h o "hoje" do servidor
// já era o dia seguinte.
//
// O fuso é NOMEADO de propósito. Hoje o Brasil não tem horário de verão e o
// deslocamento é sempre -03:00, mas fixar esse número no código seria um bug
// dormente pro dia em que ele voltar — com o nome, o próprio Intl passa a
// devolver o offset certo sem ninguém mexer aqui.
const FUSO_AGENDAMENTO = "America/Sao_Paulo";

// Mesmo idioma de paraDataHorarioBrasilia (lib/googleCalendarImportacao.js):
// en-CA + formatToParts + hourCycle h23, que entrega os componentes já no
// formato que o resto do app usa ("YYYY-MM-DD" e hora 0–23). Instanciado UMA
// vez — o construtor de Intl.DateTimeFormat é caro e isto roda por slot.
const FORMATADOR_FUSO = new Intl.DateTimeFormat("en-CA", {
...
// Instante absoluto -> componentes de PAREDE em America/Sao_Paulo.
function partesNoFuso(instante) {
  const mapa = Object.fromEntries(
    FORMATADOR_FUSO.formatToParts(instante).map((p) => [p.type, p.value])
  );
  return {
...
// Deslocamento (ms) do fuso do agendamento num dado instante: quanto a parede
// de São Paulo está adiantada/atrasada em relação a UTC (hoje, -3h). Medido a
// cada chamada, nunca constante — ver o comentário de FUSO_AGENDAMENTO.
function deslocamentoDoFuso(instante) {
  const { data, hora, minuto, segundo } = partesNoFuso(instante);
  const [ano, mes, dia] = data.split("-").map(Number);
  const paredeComoUTC = Date.UTC(ano, mes - 1, dia, hora, minuto, segundo);
  return paredeComoUTC - instante.getTime();
}

// "YYYY-MM-DD" + "HH:MM" (parede de São Paulo) -> Date do instante ABSOLUTO
// correspondente. Substitui a antiga construirDataHora, que montava o Date
// com os componentes LOCAIS do processo — certa no navegador da cliente,
// errada em três horas no servidor.
//
// Duas passadas porque o deslocamento é medido, não assumido: a primeira
// corrige pelo offset do palpite inicial, a segunda confere o offset já no
// instante corrigido. Sem horário de verão as duas dão no mesmo resultado;
// com ele, é a segunda que acerta as datas de transição.
function instanteNoFuso(data, horario) {
  const [ano, mes, dia] = data.split("-").map(Number);
  const [h, m] = horario.split(":").map(Number);
  const paredeComoUTC = Date.UTC(ano, mes - 1, dia, h, m);

  const palpite = new Date(paredeComoUTC - deslocamentoDoFuso(new Date(paredeComoUTC)));
  return new Date(paredeComoUTC - deslocamentoDoFuso(palpite));
}

// "YYYY-MM-DD" -> o dia seguinte, na mesma string. A aritmética é em UTC (e
// não new Date(ano, mes - 1, dia + 1), que passaria pelo fuso do processo) —
// aqui o Date serve só pra normalizar virada de mês/ano.
...
// Linha de `ausencias` que é uma exclusividade de serviço (restrição do
// próprio serviço), e não um bloqueio/liberação. Fica FORA de excecoesDoDia/
// aplicarExcecoes, que trataria qualquer tipo_registro desconhecido como
// bloqueio.
function ehExclusividade(a) {
  return a.tipo_registro === "exclusividade_servico";
}

// Regras de exclusividade do serviço consultado que estão ATIVAS nesta data.
// "Ativa" depende só do PERÍODO, não do dia da semana: 'recorrente'
// (permanente) sempre; 'recorrente_periodo' quando a data cai em
// data_inicio..data_fim. Uma regra ativa num dia da semana NÃO marcado é
// justamente o que tira o serviço desse dia (ver aplicarExclusividades).
function exclusividadesAtivasDoServico(ausencias, servicoId, data) {
  return (ausencias ?? []).filter((a) => {
    if (!ehExclusividade(a)) return false;
    if (String(a.servico_id) !== String(servicoId)) return false;
    if (a.tipo === "recorrente") return true;
    if (a.tipo === "recorrente_periodo") {
      return a.data_inicio <= data && data <= a.data_fim;
    }
    return false;
  });
}
...
function aplicarExcecoes(conjunto, excecoes, duracaoMin) {
  ...
  for (const ex of excecoes) {
    // Só tipo_registro === "ausencia" (default, cobre linhas antigas sem a
    // coluna) bloqueia — liberação já foi tratada acima, e exclusividade nem
    // chega aqui (ver aplicarExclusividades).
    if ((ex.tipo_registro ?? "ausencia") !== "ausencia") continue;
...
// Restringe o conjunto FINAL de um profissional (depois de aplicarExcecoes)
// às regras de exclusividade ATIVAS do serviço consultado, mutação in-place.
// `regrasAtivas` já vem filtrada por serviço, profissional e período (ver
// exclusividadesAtivasDoServico):
//   - nenhuma regra ativa → conjunto intocado (serviço 100% normal);
//   - alguma ativa        → o conjunto vira a INTERSEÇÃO com os horários
//                           marcados pra ESTE dia da semana (vazio se o dia
//                           não foi marcado). Nunca adiciona: um horário
//                           marcado que não está mais na agenda (ou que uma
//                           ausência removeu) continua fora.
// Casamento pelo INÍCIO exato do horário ("HH:MM"), normalizando o
// "HH:MM:SS" do Postgres — os horários marcados vêm da própria agenda do
// profissional (ver SecaoExclusividade). Devolve o Set dos horários
// removidos (o snapshot do admin anota esses com motivo próprio).
function aplicarExclusividades(conjunto, regrasAtivas, dia) {
  const removidos = new Set();
  if (regrasAtivas.length === 0) return removidos;

  const permitidos = new Set(
    regrasAtivas
      .filter((r) => r.dia_semana === dia && r.hora_inicio)
      .map((r) => paraHHMM(r.hora_inicio))
  );
  for (const slot of [...conjunto]) {
    if (permitidos.has(slot)) continue;
...
// Status mensal da agenda (tabela `janela_agendamento_meses`) -> Map
// "AAAA-M" -> linha, no formato que mesAgendavel/dataAgendavelComMes esperam
// (ver lib/janelaAgendamento.js, que é quem aplica a regra e continua sem
// tocar em supabase). Consulta INDEPENDENTE da data, igual à de restrições
// aqui embaixo: carrega o salão inteiro uma vez e decide em memória, mês a mês.
//
// DOIS retornos vazios diferentes, e a diferença importa (ver o bloco de
// fail-closed em lib/janelaAgendamento.js):
//   consulta OK sem linhas -> Map vazio: o salão não configurou nada, e com a
//                             regra fail-closed isso FECHA todos os meses.
//   consulta falhou        -> Map marcado por marcarMesesJanelaIndisponiveis:
//                             mesAgendavel passa a não filtrar por mês
//                             (fail-open). Uma queda de rede/Supabase não pode
//                             fechar a agenda pública de um salão pagante.
// O catch cobre o caso em que o cliente REJEITA em vez de devolver `error`
// (falha de rede é assim) — sem ele a exceção subiria pra quem chama, que na
// UI vira promise rejeitada e mapa preso no valor inicial, ou seja o mesmo
// falso "tudo fechado" por outro caminho.
//
// `estabelecimentoId` ausente segue devolvendo Map vazio SEM marca: é erro de
// programação de quem chamou, não falha de leitura, e todos os pontos de
// chamada já barram esse caso antes.
export async function buscarMesesJanela(estabelecimentoId) {
  if (!estabelecimentoId) return new Map();
  ...
  try {
    ...
    if (resposta.error) return marcarMesesJanelaIndisponiveis();
    data = resposta.data;
  } catch {
    return marcarMesesJanelaIndisponiveis();
  }

  const mapa = new Map();
  for (const linha of data ?? []) {
    mapa.set(chaveMesJanela(linha.ano, linha.mes), linha);
  }
  return mapa;
}
...
//   servicoId       – serviço da consulta (as exclusividades DELE restringem
//                     os candidatos, ver aplicarExclusividades)
...
//                     (janela_agendamento_meses), guardado na base pra que
//                     calcularVagasDoMes decida mês a mês sem reconsultar
...
// qualquer consulta pesada: quando a data pedida não é agendável (status do
// mês, ou janela_agendamento_fim nos meses sem status — ver
// dataAgendavelComMes), devolve `foraDaJanela: true` logo depois de ler o
// estabelecimento e o status mensal, sem disparar as demais consultas. Quem calcula o mês não
...
async function carregarBaseDisponibilidade({
  ...
  // Status mensal da agenda (janela_agendamento_meses). Carregado ANTES da
  // checagem abaixo porque é ele quem decide nos meses configurados — a data
  // única só vale nos meses sem registro (ver dataAgendavelComMes).
  const mesesJanela = await buscarMesesJanela(estabelecimentoId);

  if (
    dataParaJanela &&
    !dataAgendavelComMes(dataParaJanela, estab, mesesJanela, etiquetaClienteId)
  ) {
    return { ...vazio, estab, mesesJanela, foraDaJanela: true };
  }
  ...
  if (
    ...
    return { ...vazio, estab, mesesJanela, restricoes, restritoPorEtiqueta: true };
  }
  ...
  const { data: ausenciasRows, error: erroAusencias } = await supabase
    .from("ausencias")
    .select(
      "profissional_id, tipo, tipo_registro, dia_semana, data_inicio, data_fim, dia_inteiro, hora_inicio, hora_fim, servico_id"
    )
...
// expediente/modo normal ('fora_do_modo'), e `removidosPorExclusividade` —
// Map profissional_id -> Set dos horários que só saíram porque o serviço
// consultado está restrito a outros horários ('exclusividade_servico').
function candidatosDoDia(base, data, comSnapshot = false) {
  ...
  // Exclusividades ficam FORA de aplicarExcecoes (não são bloqueio nem
  // liberação) e têm filtro de data próprio: ativas pelo período, qualquer
  // que seja o dia da semana (ver exclusividadesAtivasDoServico).
  const excecoesAplicaveis = excecoesDoDia(
    base.ausenciasRows.filter((a) => !ehExclusividade(a)),
    dia,
    data
  );
  const exclusividadesAtivas = exclusividadesAtivasDoServico(
    base.ausenciasRows,
    base.servicoId,
    data
  );
  ...
  // Exclusividades por último, sobre o que a agenda (já com exceções)
  // realmente oferece — cada profissional só pelas regras DELE.
  const removidosPorExclusividade = comSnapshot ? new Map() : null;
  for (const profissionalId of base.profissionalIds) {
    ...
    const regras = exclusividadesAtivas.filter(
      (a) => a.profissional_id === profissionalId
    );
    const removidos = aplicarExclusividades(conjunto, regras, dia);
    removidosPorExclusividade?.set(profissionalId, removidos);
    candidatos.set(profissionalId, conjunto);
  }

  return { candidatos, candidatosAntes, removidosPorExclusividade };
}
...
function montarMapaDoDia({
  ...
  if (contexto === "admin") {
    ...
    for (const profissionalId of base.profissionalIds) {
      ...
      const porExclusividade =
        removidosPorExclusividade?.get(profissionalId) ?? new Set();
      ...
      for (const slot of gradeCompleta) {
        ...
        } else if (porExclusividade.has(slot)) {
          // Antes de `antes`: um horário de liberação (fora do snapshot) que
          // a restrição cortou também é exclusividade, não fora_do_modo.
          entrada.bloqueados.push({ profissionalId, motivo: "exclusividade_servico" });
...
// removeu), 'exclusividade_servico' (o serviço está restrito a outros
// horários por uma regra de exclusividade ativa) ou 'fora_do_modo' (o slot
// nunca seria candidato — fora do expediente/lista fixa/dia da semana
// configurados). A ÚNICA coisa que
...
export async function calcularVagasPorHorario({
  ...
  const { candidatos, candidatosAntes, removidosPorExclusividade } = candidatosDoDia(
    base,
    data,
    contexto === "admin"
  );
...
export async function calcularVagasDoMes({
  ...
  for (let d = 1; d <= diasNoMes; d++) {
    ...
    // dataAgendavelComMes/diaLiberadoPorEtiqueta em
...
//
// O INSTANTE de referência é `agora` (default `new Date()`) — no client, a
// hora do navegador (só filtro de UX); a garantia de verdade vem de rodar
...
//
// Esse instante é absoluto e independe de fuso; o que dependia era a LEITURA
// dele. Todo "que dia/hora é" daqui pra baixo passa por partesNoFuso/
// instanteNoFuso, fixos em America/Sao_Paulo, então o resultado é o mesmo com
// o processo em UTC (Vercel) ou em BRT (navegador da cliente, `next dev`).
// `agora` é injetável pra tornar a função testável sem mexer no relógio do
// sistema — nenhum ponto de chamada precisa passá-lo.
//
...
export function filtrarPorAntecedenciaMinima(
  ...
  // Parede de São Paulo no instante de referência: é daqui que saem "hoje",
  // "amanhã" e "que horas são" — não dos getters locais do processo.
  const parede = partesNoFuso(agora);
  const ehHoje = data === parede.data;

  let cutoffBloqueiaODiaSeguinte = false;
  if (cutoffAtivo) {
    cutoffBloqueiaODiaSeguinte =
      data === proximoDiaISO(parede.data) &&
      parede.hora >= Number(estabelecimento.cutoff_dia_seguinte_hora);
  }

  const resultado = {};
  for (const horario of Object.keys(horariosDisponiveis)) {
    const instanteDoSlot = instanteNoFuso(data, horario).getTime();
    const bloqueado =
      (ehHoje && instanteDoSlot < agora.getTime()) ||
      (antecedenciaAtiva &&
        (instanteDoSlot - agora.getTime()) / (1000 * 60 * 60) < Number(antecedenciaHoras)) ||
...
// Teto de segurança da varredura de calcularPrimeiroMesComVaga, e desde o
// status mensal da agenda o ÚNICO limite dela: não existe mais uma data final
// que corte o loop antes (a antiga janela_agendamento_fim cortava). Sem teto,
// um salão com todos os meses fechados varreria pra sempre — com ele, a busca
// olha no máximo 12 meses à frente e desiste.
...
//                   corte do dia seguinte, os mesmos que diasSemVagaDoMes
//                   usa. Omitido, cai no que a base carregou.
//
...
export async function calcularPrimeiroMesComVaga({
  ...
  for (let varridos = 0; varridos < MAX_MESES_VARREDURA; varridos++) {
    // Mês fechado pelo status mensal da agenda: nenhum dia dele é oferecível,
    // então pula sem gastar a consulta de `agendamentos` do mês.
    //
    // Diferente do corte por janela_agendamento_fim que existia aqui, um mês
    // fechado NÃO encerra a varredura: os meses agora são independentes entre
    // si (dezembro pode estar aberto com novembro fechado), e parar no
    // primeiro fechado esconderia do calendário público um mês que a dona
    // abriu. Quem encerra é só o teto MAX_MESES_VARREDURA.
    //
    // Usa o MESMO Map já carregado na base (base.mesesJanela), nunca uma
    // consulta por mês — a varredura acontece toda em memória.
    if (
      !mesAgendavel(ano, mes, dadosEstabelecimento, base.mesesJanela, etiquetaClienteId)
    ) {
      mes += 1;
      if (mes > 12) {
        mes = 1;
