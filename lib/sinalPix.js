...
// Os dois degraus da cascata, SEM o corte de `sinal_regra`: o meio de cobrança
// que o salão tem configurado de verdade, independente de a regra pedir sinal.
// Fonte única dos degraus — calcularStatusSinalPix aplica o corte da regra por
// cima disto, metodoDisponivelSinalPix devolve isto direto.
function metodoConfigurado({ metodo_cobranca_pix, sinal_chave_pix, abacatepayConectado }) {
...
//
// `cobraAlguem` é o corte de "este salão cobra sinal de alguém?", e existe
// porque `sinal_regra` deixou de responder isso sozinho: uma regra especial de
// período ou de serviço (lib/sinalRegra.js) liga a cobrança num salão de
// `sinal_regra = 'desligado'`, e sem este parâmetro a função sairia na primeira
// linha justamente no salão que MAIS precisa do aviso — o que vai cobrar em
// dezembro sem ter chave Pix nem credencial. Quem sabe a resposta é
// `salaoPodeCobrarSinal`, e ela é PASSADA de fora, nunca importada: este
// módulo responde por capacidade de cobrança (a cascata), não por regra de
// negócio, e importar lib/sinalRegra.js aqui faria a cascata depender de
// serviço e data. Omitido, cai no critério de sempre.
export function calcularStatusSinalPix({
  sinal_regra,
  metodo_cobranca_pix,
  sinal_chave_pix,
  abacatepayConectado,
  cobraAlguem = undefined,
} = {}) {
  ...
  if (!(cobraAlguem ?? sinal_regra !== "desligado")) {
    return { metodoEfetivo: "desligado", rebaixado: false };
  }

  const metodo = metodoConfigurado({
    metodo_cobranca_pix,
    sinal_chave_pix,
    abacatepayConectado,
  });

  // Fundo do poço ('desligado' aqui): a regra do salão pedia cobrança e não
  // sobrou nenhum meio de cobrar. Único caso de `rebaixado: true`.
  return { metodoEfetivo: metodo, rebaixado: metodo === "desligado" };
}
...
//
// `cobraAlguem` é repassado (ver calcularStatusSinalPix): quem chama do /admin
// entrega `salaoPodeCobrarSinal(estabelecimento)`, pra que uma regra especial
// ativa num salão de padrão desligado também acenda o vermelho.
export function sinalPixRebaixado(estabelecimento, cobraAlguem = undefined) {
...
// Método que o salão CONSEGUE cobrar, ignorando `sinal_regra`. Mesma cascata
// (abacatepay conectado -> chave Pix -> 'desligado'), só sem o corte da regra.
// Serve a quem precisa cobrar mesmo com a regra desligada:
//   - a regra da Lista de Bloqueio (ver precisaSinal em FormularioAgendamento),
//     que força o sinal de uma cliente específica fora da regra normal;
//   - ConfirmacaoSinal, que só monta pra uma linha JÁ em "aguardando_sinal" —
//     a cobrança existe, a pergunta é só por onde pagá-la.
// Com a regra ligada devolve exatamente o mesmo que metodoEfetivoSinalPix.
export function metodoDisponivelSinalPix(estabelecimento) {
  return metodoConfigurado({
    metodo_cobranca_pix: estabelecimento?.metodo_cobranca_pix,
    sinal_chave_pix: estabelecimento?.sinal_chave_pix,
    abacatepayConectado: estabelecimento?.abacatepay_conectado,
  });
