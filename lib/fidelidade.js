...
export async function contarServicosFidelidade(clienteId, estabelecimentoId) {
  ...
  const { data: agendamentos } = await supabase
    .from("agendamentos")
    .select("data, horario, duracao_min, status, papel_reserva, servicos(eh_manutencao)")
    .eq("estabelecimento_id", estabelecimentoId)
  ...
  return (agendamentos ?? []).filter((item) => {
    ...
    // A etapa anterior de um par não é visita: o par vale uma.
    if (item.papel_reserva === "anterior") return false;
