...
// colorId "8" (Graphite) marcado em produção como cancelamento manual — a
// dona usa essa cor pra sinalizar horário liberado, não um atendimento real.
// Tratado igual a um evento cancelado: nunca vira candidato.
const COR_ID_CANCELADO = "8";

// colorId "7" (Peacock) tem confirmação real de um agendamento genuíno
// (evento "manutenção lari" em produção). Ausência de colorId é a categoria
// dominante confirmada (~4386 de ~5256 eventos reais). Qualquer outra cor
// (9, 11, etc.) ainda não tem confirmação de ser atendimento — fica como
// "pessoal" até ter dado real que justifique reclassificar.
const COR_ID_SERVICO = "7";
...
// então só interessa data/horário/duração/título bruto + a categoria
// derivada do colorId (ver COR_ID_SERVICO/COR_ID_CANCELADO). Eventos
// cancelados (por status ou por COR_ID_CANCELADO), de dia inteiro (sem
// horário) ou com título "só hora" (ver ehTituloSoHora) são ignorados — o
// schema de agendamentos exige um horário específico, título "só hora" não
// carrega nenhuma informação real de cliente/serviço, e COR_ID_CANCELADO
// representa horário liberado, não ocupado. A classificação "servico" vs
// "pessoal" é só um ponto de partida — a dona pode corrigir manualmente caso
// a caso (toggle "Marcar como atendimento"/"Marcar como pessoal") quando a
// regra errar.
export function montarCandidatos({ eventos, idsExcluidos }) {
  return eventos
    ...
    .filter((evento) => evento.colorId !== COR_ID_CANCELADO)
    .map((evento) => {
      ...
      return {
        ...
        categoria: !evento.colorId || evento.colorId === COR_ID_SERVICO ? "servico" : "pessoal",
      };
    });
}
