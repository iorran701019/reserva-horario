...
// Corta um texto antes do primeiro "(" — descarta a parte
...
// ('pessoal') não vira candidato, só entra em eventosIgnorados (ver GET em
// app/api/google-calendar/importar), listado no modal pra dar um sinal caso um
...
export function montarCandidatos({ eventos, idsExcluidos, servicos }) {
  ...
  const candidatos = [];
  // Só referência pra decisão manual no modal (título/data/horário) — nunca
  // vira candidato nem é gravado em calendar_import_ignorados.
  ...
  return { candidatos, eventosIgnorados };
}
