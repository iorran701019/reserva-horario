...
//
// status "confirmado"/"concluido" sem google_event_id -> cria evento
// status "confirmado"/"concluido" com google_event_id -> atualiza evento
// status "cancelado" com google_event_id               -> apaga evento
// qualquer outro caso                                  -> não faz nada
//
// "concluido" (gravado pelo cron depois de confirmado_expira_horas, ou pela
// dona em "Aguardando Conclusão", na aba Pendentes) é irmão de "confirmado": o atendimento
// aconteceu, então o evento FICA na agenda. Antes, qualquer status diferente
// de "confirmado" apagava — a virada automática pra concluido sumiria com o
// histórico da agenda da dona. Só cancelado (inclusive falta, que é
// cancelado + nao_compareceu) apaga.

import { antesDoParentese } from "./googleCalendarImportacao";
import { ehStatusSucesso } from "./particao";
...
export async function sincronizarEventoComGoogle({ agendamento, accessToken, supabaseAdmin }) {
  ...
  if (ehStatusSucesso(agendamento.status)) {
  ...
  if (agendamento.status === "cancelado" && agendamento.google_event_id) {
