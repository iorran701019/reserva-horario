...
// Formata o telefone pra EXIBIÇÃO: "(xx) xxxxx-xxxx" (celular, 11 dígitos) ou
// "(xx) xxxx-xxxx" (fixo, 10). Passa antes por paraNumeroWhatsApp e tira o DDI,
// pra aceitar qualquer formato que o link já aceita (com ou sem 55, máscara,
// zero à esquerda). Fora desses tamanhos devolve null — quem chama esconde a
// linha em vez de mostrar um número mal formatado.
export function formatarWhatsappExibicao(telefone) {
  const nacional = paraNumeroWhatsApp(telefone).slice(2);
...
const PADRAO_CONFIRMACAO =
  "Olá {nome_cliente}! Seu agendamento de {servico} no dia {data} às {horario} está agendado. Será um prazer lhe atender! ✅";
...
// Mensagem livre de dúvida geral (botão fixo do ContatoDono — ícone do
// WhatsApp + nome do responsável —, visível em qualquer tela do fluxo
// público). Sem variáveis.
