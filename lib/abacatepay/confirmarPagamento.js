...
export async function confirmarPagamentoPix(agendamentoId, supabaseAdmin) {
  const { data: agendamento, error: erroAgendamento } = await supabaseAdmin
    ...
    // especiais de sinal: o valor de reserva do fallback abaixo depende do par
    // (serviço, data do atendimento), não só do salão (ver lib/sinalRegra.js).
    .select(
      "id, estabelecimento_id, servico_id, data, status, abacatepay_pago_em, sinal_valor_centavos, servicos(eh_manutencao)"
    )
    .eq("id", agendamentoId)
    .maybeSingle();
  ...
  if (agendamento.sinal_valor_centavos == null) {
    ...
    // Mesma resolução da rota de gerar-cobranca, pelo mesmo módulo: uma regra
    // especial pode cobrar por serviço ou por período, e este fallback existe
    // justamente pros casos em que o número não foi gravado lá. Ler o
    // `sinal_valor_centavos` do salão cru aqui gravaria um valor diferente do
    // que a cliente viu e pagou.
    //
    // Erro na leitura das regras cai em lista vazia (o padrão do salão), igual
    // ao resto do caminho: este bloco inteiro é rede de segurança, e o dinheiro
    // já entrou.
    ...
    if (erroRegrasSinal) {
      console.error(
        "Falha ao buscar regras especiais de sinal pra confirmar o Pix",
        agendamentoId,
        erroRegrasSinal
      );
    }
