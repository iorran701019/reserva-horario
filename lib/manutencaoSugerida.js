...
// já passou de servicos.prazo_manutencao_dias (dele) E o salão tiver
// manutencao_valor_cheio_apos_prazo=true, cobra o preco_centavos do serviço
// de ORIGEM. Caso contrário (dentro do prazo, sem histórico, salão não exige
// valor cheio, ou o serviço nem é manutenção), cobra o preco_centavos normal
// da manutenção — comportamento de hoje, inalterado.
//
...
export async function calcularPrecoManutencao(
  ...
  console.log("[DEBUG calcularPrecoManutencao] estabelecimento:", {
    estabelecimentoId,
    manutencaoValorCheioAposPrazoRaw: resEstab.data?.manutencao_valor_cheio_apos_prazo,
    servicoOrigemId: servicoManutencao.servico_origem_id,
  });
  ...
  console.log("[DEBUG calcularPrecoManutencao] atendimento de referência:", {
    dataAtendimentoReferencia: concluido?.data ?? null,
    prazoManutencaoDiasOrigem: servicoOrigem.prazo_manutencao_dias,
  });

  if (!concluido) return precoNormal;

  const diasDecorridos = Math.round(
    (hojeLocal() - somarDias(concluido.data, 0)) / DIA_EM_MS
  );

  console.log("[DEBUG calcularPrecoManutencao] decisão:", {
    diasDecorridos,
    vencido: diasDecorridos > servicoOrigem.prazo_manutencao_dias,
  });
