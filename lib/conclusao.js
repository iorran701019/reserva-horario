...
// Só o clique manual chega aqui (o cron nunca passa por este arquivo), então
// toda chamada marca editado_manualmente_em.
...
// `statusAnterior` é o status do item ANTES da ação: só um "concluido" que
// vira falta (botão "Editar" do Histórico) conta como edição manual e grava
// editado_manualmente_em; a marcação original, vinda de "Aguardando
// Conclusão" (confirmado), não.
export async function marcarNaoCompareceu(agendamentoId, extras = {}, statusAnterior = null) {
  const marcaEdicao =
  ...
  const { data, error } = await supabase
    .from("agendamentos")
    .update({ ...extras, ...marcaEdicao, nao_compareceu: true, status: "cancelado" })
    .eq("id", agendamentoId)
    .in("status", STATUS_SUCESSO)
    .select("id");
...
export async function registrarValorCobrado(agendamentoId, valorCentavos, extras = {}) {
  // Só roda sobre concluídos, então toda chamada é uma edição manual.
  const { data, error } = await supabase
    .from("agendamentos")
    .update({
      ...extras,
