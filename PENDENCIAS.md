...
- **Deploy em produção:** se não foi concluído no fechamento da Sessão 73, aplicar em produção, nesta ordem, depois de conferir que as funções de produção batem com `main`: `sql/segunda_data_reserva_grupo.sql`, `sql/rpc_criacao_par.sql`, `sql/rpcs_leitura_cliente_par.sql`, `sql/rpc_cancelar_par.sql`, `sql/fidelidade_base_cliente_par.sql`. Só então fazer o merge para `main`. Sem as colunas, o `/admin`, a aba Serviços e a criação de agendamento quebram com 42703.
