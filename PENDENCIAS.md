...
- **Riscos aceitos, registrados para não serem redescobertos:** (1) as RPCs de leitura do painel da cliente usam o telefone como único fator — quem souber o número de outra pessoa vê os agendamentos dela naquele salão (mesmo modelo de `cliente_buscar_por_whatsapp`; segundo fator é decisão de produto); (2) as RPCs por id e a rota de upload do comprovante confiam no `uuid` aleatório do agendamento como "senha" — quem tiver o id pode cancelar/declarar sinal/reenviar o comprovante daquele agendamento; (3) `agendamento_criar` valida pertencimento e limites, mas não regra de negócio — uma chamada direta à RPC ainda reserva fora da janela/antecedência e escolhe entre `pendente`/`aguardando_sinal`; a revisão manual da dona nos Pendentes é a trava final.
- **`sql/` do repositório diverge do banco em vários arquivos** — repetidamente enganou a investigação na Sessão 71: `sql/pendencias_admin.sql` declara `agendamento_id bigint` (no banco é `uuid`); `sql/fidelidade_resgates.sql` declara `criado_em` (no banco é `resgatado_em`) e não tem a policy pública que existia em produção; `sql/agendamentos_anon_update_policy.sql` descreve uma policy que não existe mais (removida na Etapa 5); as policies do bucket `comprovantes-pix` nunca estiveram no repo. Atualizar esses arquivos para o estado real (ou marcá-los como históricos) numa sessão curta dedicada.
...
- **Base da RPC já existe (Sessão 71):** `agendamento_criar` (`sql/rpc_criacao_agendamento.sql`) foi desenhada como a unidade "uma pessoa, uma linha", sem bloco EXCEPTION e sem estado entre chamadas. Um futuro `agendamento_criar_grupo(...)` a chama N vezes no próprio corpo plpgsql — as N linhas nascem na mesma transação e, se uma bater na exclusion constraint (`agendamentos_sem_sobreposicao`), todas somem juntas. A única mudança de assinatura prevista em `agendamento_criar` é um `p_grupo_id` opcional quando a coluna existir.
...
- Sessão dedicada ao Programa de Fidelidade (botão "agendar brinde" sem handler, expiração de 12 meses, notificação). A leitura pública da fidelidade passou pela RPC `fidelidade_base_cliente` na Sessão 71 (resolve o cliente pelo telefone normalizado, não pelo `clienteId`).
...
- **Agendamento em grupo pra maquiadoras** — ver seção própria em "Em aberto". Desenho pronto, base da RPC já existe, aguardando alinhamento de negócio com a Laryssa antes de virar branch.
